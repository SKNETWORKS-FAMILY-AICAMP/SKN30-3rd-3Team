...
### 🚀 향후 고도화 과제
- **무정보/검색 실패 시의 거절 강인성 확보:** 검색 결과가 완전히 비어있을 때 LLM Generator가 아예 잡담 모드로 흐르거나 일반 지식으로 얼버무리지 않고, 데이터셋에 맞춤화된 전문 거절 및 추가 정보 요청(예: "현재 등록된 가이드북에 관련 정보가 확인되지 않습니다. 식물 상태 사진과 물주기 주기를 추가해 주시면...") 패턴을 명확히 구현해야 합니다.
- **다중 식물 비교 지원을 위한 데이터 청킹 보완:** 단일 식물 도큐먼트 위주에서 질병/해충 간의 증상 비교 데이터셋 및 청크 범위를 보강하여, 다중 문서 종합 질문에 대한 검색 정확도를 더욱 끌어올려야 합니다.
